...
def _wait_for_session_cookies(redirect_host, redirect_port):
...
class OpenIDConnect(oidc._OidcBase):
    ...
    def _get_session_cookies(self, location):
        ...
        cookies = _wait_for_session_cookies(self.redirect_host, self.redirect_port)
        return cookies


    def _get_keystone_token(self, session, cookie):
        """Exchange the access_token for a keystone token

        :param obj session: keystoneauth1.session.Session
        :param str cookie: the cookie headers containing the mod_auth_openidc
        ...
        auth_response = session.get(self.federated_auth_url,
                                     headers=headers,
                                     authenticated=False)
    ...
    # def get_auth_ref(self, session):
        ...
        1.- We need to establish session cookies with mod_auth_openidc

        2.- Use these session cookies to obtain our keystone token
